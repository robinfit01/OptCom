	...
	if (c == 'a')
	{
		...
		CObjectVector<CDirItem> dirItems;
		{
			...
			for (i = 3; i < numArgs; i++)
			{
				...
				char *nameStr = (char*)malloc(strlen(args[i]) - temp);

				for (int j = 0; j < strlen(args[i]) - temp - 1; j++)
				{
					nameStr[j] = args[i][temp + 1 + j];
					nameStr[j + 1] = 0;
				}
				...
				//----Modified at 16/6/11
				//di.Name = fs2us(name);
				di.Name = fs2us(CmdStringToFString(nameStr));
...
#ifdef DEBUG
	afxDump << strlen(FileName);
	afxDump << *FileName << *(FileName+1) << *(FileName+2) << *(FileName+3);
#endif // DEBUG
